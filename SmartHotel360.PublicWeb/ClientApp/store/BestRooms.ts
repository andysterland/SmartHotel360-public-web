...
interface ReceiveFeaturedAction { type: 'RECEIVE_FEATURED_ACTION', list: BestRoom[] }
interface RequestFilteredAction { type: 'REQUEST_FILTERED_ACTION' }
interface ReceiveFilteredAction { type: 'RECEIVE_FILTERED_ACTION', list: BestRoom[] }
...
export const actionCreators = {
    requestFeatured: (): AppThunkAction<KnownAction> => (dispatch, getState) => {
        ...
        let fetchTask = fetch(`${settings.urls.hotels}Featured`)
            .then(response => response.json() as Promise<BestRoom[]>)
            .then(data => {
    ...
    requestFiltered: (): AppThunkAction<KnownAction> => (dispatch, getState) => {
        ...
        //    });
        let fetchTask = fetch(`http://localhost:62487/api/Hotels/best`, { method: 'GET' })
            .then(response => response.json() as Promise<BestRoom[]>)
            .then(data => {
                dispatch({ type: 'RECEIVE_FILTERED_ACTION', list: data });
            });
